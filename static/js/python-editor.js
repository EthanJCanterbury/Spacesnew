...
// Pygame tab switching functionality
document.addEventListener('DOMContentLoaded', function() {
    // Set up tabs for switching between console and pygame output
    ...
    outputTabs.forEach(tab => {
        tab.addEventListener('click', function() {
            ...
            // Hide all panels
            outputPanels.forEach(panel => panel.style.display = 'none');
...
window.runPythonCode = function() {
    ...
    const code = pythonEditor.getValue();
    const outputConsole = document.getElementById('pythonOutput');
    const pygameCanvas = document.getElementById('pygameCanvas');
    const pygameMessage = document.getElementById('pygameMessage');

    // Show loading indicator in console
    outputConsole.innerHTML = 'Running code...\n';
    
    // Check if code contains pygame-related imports
    const hasPygame = code.includes('import pygame') || code.includes('from pygame');
    
    // If pygame is detected, switch to pygame tab
    if (hasPygame) {
        const pygameTab = document.querySelector('.output-tab[data-target="pygame"]');
        if (pygameTab) {
            pygameTab.click();
        }
        
        pygameCanvas.style.display = 'none';
    ...
    .then(response => response.json())
    .then(data => {
        // Handle regular console output
        if (data.error) {
            outputConsole.innerHTML = `<span class="console-error">${data.output}</span>`;
            
            // Also show error in pygame panel if it's active
            if (hasPygame) {
                pygameCanvas.style.display = 'none';
                pygameMessage.textContent = `Error: ${data.output}`;
            }
        } else {
            outputConsole.innerHTML = data.output;
            ...
            if (data.pygame_url) {
                pygameMessage.style.display = 'none';
                pygameCanvas.style.display = 'block';
                
                // Create iframe for pygame display if using remote VNC
                const iframeContainer = document.createElement('div');
                iframeContainer.style.width = '100%';
                iframeContainer.style.height = '100%';
                
                const iframe = document.createElement('iframe');
                iframe.src = data.pygame_url;
                iframe.style.width = '100%';
                iframe.style.height = '100%';
                iframe.style.border = 'none';
                
                // Replace canvas with iframe
                pygameCanvas.parentNode.insertBefore(iframeContainer, pygameCanvas);
                iframeContainer.appendChild(iframe);
                pygameCanvas.remove();
            } else if (hasPygame) {
                pygameCanvas.style.display = 'none';
                pygameMessage.textContent = 'Pygame detected but no graphical output was produced. Make sure you initialize a display and call pygame.display.flip() or pygame.display.update().';
            }
        }
    })
    .catch(error => {
        ...
        if (hasPygame) {
            pygameCanvas.style.display = 'none';
            pygameMessage.textContent = `Failed to run pygame: ${error}`;
        }
    });
};
