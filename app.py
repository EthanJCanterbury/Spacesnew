...
def create_pixi_site():
    try:
        ...
        default_pixi_content = '''<!doctype html>
<html>
  <head>
  ...
  <body>
    <script type="module">
      ...
      app.ticker.add((ticker) => {
        ...
        site = Site(name=name,
                    user_id=current_user.id,
                    html_content=default_pixi_content,
                    site_type='pixi')
        db.session.add(site)
        db.session.commit()
        ...
        db.session.rollback()
        return jsonify({'message': 'Failed to create Pixi game'}), 500
...
def pixi_editor(site_id):
    try:
        ...
        return render_template('pixieditor.html',
                               site=site,
                               additional_scripts=socket_join_script)
    except Exception as e:
