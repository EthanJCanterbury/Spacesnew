...
def run_python_with_pygame():
    ...
    use_pygame = data.get('use_pygame', False)
    ...
    try:
        with open('allowed_imports.json', 'r') as f:
            allowed_imports_data = json.load(f)
            allowed_imports = allowed_imports_data.get('allowed_imports', [])
    except Exception as e:
        app.logger.error(f"Error loading allowed imports: {str(e)}")
        allowed_imports = ["math", "random", "datetime", "json", "collections", 
                           "re", "string", "functools", "time", "statistics", 
                           "decimal", "fractions", "os.path", "sys", "csv", "turtle"]
    
    # Basic import check
    import_error = None
    import re
    import_pattern = re.compile(r'^\s*(?:from\s+(\w+(?:\.\w+)*)|import\s+([^,]+))')
    for line in code.split('\n'):
        match = import_pattern.search(line)
        if match:
            module = match.group(1) or match.group(2).strip().split(' ')[0].split('.')[0]
            if module not in allowed_imports:
                import_error = f"ImportError: module {module} is not allowed. Allowed modules are: {', '.join(allowed_imports)}"
                break
    ...
    result = {'output': '', 'error': None, 'pygame_output': None}
    ...
    try:
        ...
        # Check for errors that might have been printed to stderr
        errors = sys.stderr.getvalue()
        if errors:
            result['error'] = errors
            
        # For pygame, we would need to capture the display
        # This is a simplified implementation
        if use_pygame and 'pygame' in code:
            result['pygame_output'] = {
                'image_data': '/static/images/pygame_placeholder.png',
                'width': 640,
                'height': 480
            }
